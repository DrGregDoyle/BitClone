...
from src.blockchain.block import BlockHeader, Block
...
if __name__ == "__main__":
    ...
    # HeaderAndShortIds
    # --- We start from a sample block with 10 txs
    ...
    shortid_list = [
        ShortIDs(block_header, test_nonce, sample_block.txs[i].txid)
        for i in shortid_indices
    ]

    test_headerandshortid = HeaderAndShortIDs(block_header, test_nonce, shortid_list, prefilled_tx_list)
    print(f"HEADER AND SHORT IDS: {test_headerandshortid.to_json()}")
    ...
    print(f"HEADERS EQUAL: {test_headerandshortid == recovered_header}")

    # --- LOGGING
    # print(f"SAMPLE BLOCK: {sample_block.to_json()}")
