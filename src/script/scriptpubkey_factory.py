...
class ScriptPubKey:
    ...
    def __init__(self, script: bytes, script_type: str, address: Optional[str] = None, testnet: bool = False):
        """
        Initialize a ScriptPubKey with the given script, type, and address.
...
if __name__ == "__main__":
    ...
    pubkey = compress_public_key(pk_pt)

    # Create P2PK script using the factory method
    p2pk_script = ScriptPubKey.p2pk(pubkey)
    print(p2pk_script.to_json())

    # Create P2PKH script
    p2pkh_script = ScriptPubKey.p2pkh(pubkey)
