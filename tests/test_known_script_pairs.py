...
"""
Tests for the script engine using shared script vectors.
"""

from src.core import OPCODES
from src.script.scriptpubkeys import P2WPKH_Key
from src.tx import Witness
...
from tests.script_vectors import (
    ...
    build_p2tr_simple_spendpath_case,
    build_p2wpkh_case,
    build_p2wsh_case,
)
...
def test_p2pk_pair(script_engine):
    case = build_p2pk_case()
    valid_spend = script_engine.validate_script_pair(case.scriptpubkey, case.scriptsig, case.ctx)
    assert valid_spend, "Failed to validate known p2pk Script pair"
...
def test_p2sh_p2ms_pair(script_engine):
    case = build_p2sh_p2ms_case()
    valid_spend = script_engine.validate_script_pair(case.scriptpubkey, case.scriptsig, case.ctx)
    assert valid_spend, "Failed to validate known P2SH Script pair"


def test_p2sh_p2wpkh_pair(script_engine):
    case = build_p2sh_p2wpkh_case()

    p2wpkh_key = P2WPKH_Key.from_bytes(bytes.fromhex("001402c8147af586cace7589672191bb1c790e9e9a72"))
    assert OP_PUSHBYTES_22 + p2wpkh_key.script == case.scriptsig.script, (
        "P2WPKH doesn't agree with P2SH-P2WPKH ScriptSig"
    )
...
def test_p2wpkh(script_engine):
    ...
    known_witness = Witness(
        [
            bytes.fromhex(
                "3045022100c7fb3bd38bdceb315a28a0793d85f31e4e1d9983122b4a5de741d6ddca5caf82"
                "02207b2821abd7a1a2157a9d5e69d2fdba3502b0a96be809c34981f8445555bdafdb01"
            ),
            bytes.fromhex("03f465315805ed271eb972e43d84d2a9e19494d10151d9f6adb32b8534bfd764ab"),
        ]
    )

    assert known_witness == case.tx.witness[0], "Failed to parse known witness for p2wpkh tx"

    valid_spend = script_engine.validate_segwit(case.scriptpubkey, case.ctx)
    assert valid_spend, "Failed to validate known P2WPKH spend pair."


def test_p2wsh(script_engine):
    case = build_p2wsh_case()
    valid_spend = script_engine.validate_segwit(case.scriptpubkey, case.ctx)
    assert valid_spend, "Failed known P2WSH pair."


def test_keypath(script_engine):
    case = build_p2tr_keypath_case()
    valid_spend = script_engine.validate_segwit(scriptpubkey=case.scriptpubkey, ctx=case.ctx)
    assert valid_spend, "Failed to validate known key-path spend"


def test_simple_spendpath(script_engine):
    case = build_p2tr_simple_spendpath_case()
    valid_spend = script_engine.validate_segwit(case.scriptpubkey, case.ctx)
    assert valid_spend, "Failed to validate known simple script-path spend."


def test_scriptpath_spend(script_engine):
    case = build_p2tr_scriptpath_case()
    valid_spend = script_engine.validate_segwit(case.scriptpubkey, case.ctx)
    assert valid_spend, "Failed to validate known script path spend (tree)"
