...
def build_p2sh_p2ms_case() -> ScriptValidationCase:
    ...
    def _p2pkh_scriptpubkey(pubkey: bytes) -> bytes:
        h = _hash160(pubkey)
        return bytes([0x76, 0xa9, 0x14]) + h + bytes([0x88, 0xac])

    def _parse_p2pkh_scriptsig_pubkey(scriptsig: bytes) -> bytes:
        """
        ...
        items = []
        i = 0
        while i < len(scriptsig):
            n = scriptsig[i];
            i += 1
            items.append(scriptsig[i:i + n]);
            i += n
...
#     block_height=183729,
# )
# ctx = ExecutionContext(tx=tx, input_index=11, utxos=[utxo])
#
# return ScriptValidationCase("p2sh_p2ms", tx, [utxo], ctx, scriptpubkey, scriptsig)
...
def build_p2sh_p2wpkh_case() -> ScriptValidationCase:
    ...
    utxo = UTXO(
        outpoint=p2sh_p2wpkh_txid_display[::-1] + (0).to_bytes(TX.VOUT, "little"),
        amount=25552,
        scriptpubkey=scriptpubkey.script,
        block_height=826281,
    )
    ...
    return ScriptValidationCase("p2sh_p2wpkh", tx, [utxo], ctx, scriptpubkey, scriptsig)
